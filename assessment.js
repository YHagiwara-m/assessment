...
const resultDivided = document.getElementById('reslut-area');
...
assessmentButton.onclick = () => {
    ...
    if (userName.length === 0){　//名前がカラなら操作終了
        return;
    } 
...
function assesment(userName){
    ...
    for (let i = 0; i < userName.length;i++){
            Sumcode = Sumcode + userName.charCodeAt(i);
    }
    const index = Sumcode % answers.length;
